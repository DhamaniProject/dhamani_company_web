...
const AddNotificationModal: React.FC<AddNotificationModalProps> = ({
  ...
  const debouncedSkuOrUpcFilter = useDebounce(skuOrUpcFilter, 500);

  useEffect(() => {
    if (newNotification.targetReceivers === "product_users") {
      fetchProducts(debouncedSkuOrUpcFilter);
    }
  }, [debouncedSkuOrUpcFilter, newNotification.targetReceivers, fetchProducts]);
  ...
  const handleConfirmAdd = async () => {
    ...
    setError("");
    try {
      await onAdd(newNotification, skuOrUpcFilter);
    ...
    <div className="fixed top-10 right-10 z-50 w-full max-w-2xl">
      ...
      >
        ...
        <div className="flex justify-between items-center mb-4 border-b border-gray-200 pb-3">
          ...
          >
            {showConfirm ? t("addModal.confirmAdd") : t("addModal.title")}
          </h2>
          <button
            onClick={onClose}
        ...
        {showConfirm ? (
          <div className="grid gap-6">
            <h3 className="text-lg font-medium text-gray-700">
              {t("addModal.confirmAdd")}
            </h3>
            <p className="text-sm text-gray-600">
              {t("addModal.addConfirmMessage")}
            </p>
              ...
              {newNotification.targetReceivers === "product_users" && (
                <>
                  <div>
                    ...
                    >
                      <option value="">
                        {t("addModal.placeholder_product")}
                      </option>
                  ...
                  <AuthInput
                    id="newNotificationSkuOrUpc"
                    label={t("addModal.skuOrUpc")}
                    placeholder={t("addModal.placeholder_skuOrUpc")}
